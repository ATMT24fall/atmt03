...
def get_args():
    ...
    parser.add_argument(
        "--train-on-tiny", action="store_true", help="train model on a tiny dataset"
    )
    ...
    parser.add_argument(
        "--clip-norm", default=4.0, type=float, help="clip threshold of gradients"
    )
    ...
    parser.add_argument(
        "--save-dir", default="checkpoints_asg4", help="path to save checkpoints"
    )
    parser.add_argument(
        "--restore-file",
    ...
    parser.add_argument(
        "--save-interval", type=int, default=1, help="save a checkpoint every N epochs"
    )
    parser.add_argument(
        "--no-save", action="store_true", help="don't save models or checkpoints"
    )
    parser.add_argument(
        "--epoch-checkpoints", action="store_true", help="store all epoch checkpoints"
    )
...
def main(args):
    ...
    train_dataset = (
        load_data(split="train")
        if not args.train_on_tiny
        else load_data(split="tiny_train")
    )
    ...
    logging.info(
        "Built a model with {:d} parameters".format(
    ...
    for epoch in range(last_epoch + 1, args.max_epoch):
        ...
        for i, sample in enumerate(progress_bar):
            ...
            loss = criterion(
                output.view(-1, output.size(-1)), sample["tgt_tokens"].view(-1)
            ) / len(sample["src_lengths"])

            if torch.isnan(loss).any():
                logging.warning("Loss is NAN!")
                print(
                    src_dict.string(sample["src_tokens"].tolist()[0]),
                    "---",
            ...
            stats["loss"] += (
                total_loss * len(sample["src_lengths"]) / sample["num_tokens"]
            )
            ...
            progress_bar.set_postfix(
                {key: "{:.4g}".format(value / (i + 1)) for key, value in stats.items()},
                refresh=True,
            )
        ...
        logging.info(
            "Epoch {:03d}: {}".format(
                epoch,
                " | ".join(
                    key + " {:.4g}".format(value / len(progress_bar))
        ...
        if epoch % args.save_interval == 0:
            utils.save_checkpoint(
                args, model, optimizer, epoch, valid_perplexity
            )  # lr_scheduler
        ...
        if bad_epochs >= args.patience:
            logging.info(
                "No validation set improvements observed for {:d} epochs. Early stop!".format(
                    args.patience
                )
            )
            break
...
def validate(args, model, criterion, valid_dataset, epoch):
    ...
    logging.info(
        "Epoch {:03d}: {}".format(
            epoch,
            " | ".join(key + " {:.3g}".format(value) for key, value in stats.items()),
        )
